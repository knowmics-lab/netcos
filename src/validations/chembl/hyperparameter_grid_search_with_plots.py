...
def plot_scatter_panel(oof_df, config, ax):
    ...
    edgecolors = np.where(
        actual_pos & pred_pos, "tab:blue",
        np.where(actual_pos, "tab:green", np.where(pred_pos, "tab:orange", "0.6"))
    )
...
def plot_config_matrix(all_oof, all_summaries, out_file):
    """
    ...
    for ax, config_name in zip(axes, config_names):
        ...
        cfg = EvalConfig(
            ...
            cs_run_id=row["cs_run_id"],
            cs_filename=f"{row['cs_run_id']}.tsv",
...
if __name__ == "__main__":
    ...
    plot_config_matrix(all_oof, summary_df, IMG_DIR / "chembl_cv_grid_search_matrix.png")
    print("saved plot to", IMG_DIR)
