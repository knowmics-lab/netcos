...
from preprocessing_utils import get_signature_ids_list_from_cs_input,\
    get_chunk_indexes
import conf
from conf import cs_log_filename, lincs_metadata_path, make_cs_filename
...
#%% Single-conf entry point (used standalone and by the multi-conf wrapper)

def run_cs_batch_for_conf(
    *,
    cell_line=None,
    ...
    landmark_disease = landmark_disease if landmark_disease is not None else conf.landmark_disease
    landmark_drug    = landmark_drug    if landmark_drug    is not None else conf.landmark_drug
    CS_METHOD        = CS_METHOD        if CS_METHOD        is not None else conf.CS_METHOD
    cs_mith          = cs_mith          if cs_mith          is not None else conf.cs_mith
    ...
    connectivity_dataset_filename, cs_id = make_cs_filename(
        cs_mith, cs_on_LM, CS_ON_PATHWAYS, CS_METHOD, cs_out=CS_OUT)
    ...
    n_jobs = cs_batch_threads
    if n_jobs > len(drugs_list):
        raise ValueError('n_jobs:', n_jobs, ' > len(drugs_list):', len(drugs_list),
                         '! Reduce size of n_jobs')
