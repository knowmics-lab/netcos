...
source("modules/drug_signature/loader/LINCSExperimentMetaDataLoader.R")
source("modules/drug_signature/setuper/abstract_class/ExperimentMetaDataSetuper.R")

LINCSExperimentMetaDataSetuper <- R6Class(
  "LINCSExperimentMetaDataSetuper",
  inherit = ExperimentMetaDataSetuper,
  public = list(
    initialize = function(experiments_meta_data_filename) {
      private$lincsExperimentMetaDataLoader <- LINCSExperimentMetaDataLoader$new(experiments_meta_data_filename)
    },
    setup = function(perturbation_times, drugs_filter = NA, cell_name= NA ) {
      ...
      if (!obj_is_na(cell_name)) {
        experiments_meta_data <- subset(experiments_meta_data, cell_id == cell_name)
      }
      ...
      # estrae tutti gli esperimenti eseguiti sulle stesse piastre in experiments_meta_data
      ...
      experiments_meta_data <- experiments_meta_data[, c("inst_id", "rna_plate", "pert_id", "pert_iname", "pert_time", "cell_id")]
      experiments_meta_data <- experiments_meta_data[order(experiments_meta_data$inst_id),]
      experiments_env <- new.env()
      experiments_env$experiments_meta_data <- experiments_meta_data
      return(experiments_env)
    }
  ),
  private = list(
    lincsExperimentMetaDataLoader = NA
  )
)