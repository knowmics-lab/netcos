...
expected <- readRDS("test/drug_signature/data/DrugSignatureJuliaDDR1RDSExpected.Rds")
...
#saveRDS(result,"test/drug_signature/data/DrugSignatureJuliaDDR1RDSExpected.Rds")
#write.table(result, "test/drug_signature/data/DrugSignatureJuliaDDR1RDSExpected.csv", sep = ";", row.names = F)

# then
result <- result[order(result$drug),]
expected <- expected[order(expected$drug),]
test_that("DrugSignatureJuliaDDR1RDSTest", {
