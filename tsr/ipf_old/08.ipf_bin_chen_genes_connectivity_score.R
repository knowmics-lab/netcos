...
tsrLogger$log("start bin chen most significant genes connectivity score computation")

cs_parameters <- list()
cs_parameters$disease_name <- "ipf"
cs_parameters$disease_most_significant_genes_filename <- gsub(".csv", ".Rds", ipf_disease_signature_cfg$ipf_bin_chen_most_significant_genes_filename)
...
filename_no_ext <- paste0(connectivity_score_cfg$cs_base_dir, ipf_disease_signature_cfg$connectivity_score_bin_chen_filename)
...
tsrLogger$log("end bin chen most significant genes connectivity score computation")